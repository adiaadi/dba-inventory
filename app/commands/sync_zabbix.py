...
def sync_zabbix(group_names: list[str] | None = None) -> tuple[int, int]:
...
def main() -> None:
    ...
    try:
        created, updated = sync_zabbix(group_names)
    except ZabbixApiError as exc:
        raise SystemExit(f"Zabbix refresh failed: {exc}") from exc
    except RuntimeError as exc:
        raise SystemExit(str(exc)) from exc
    print(f"Zabbix refresh complete. Created: {created}. Updated: {updated}.")
